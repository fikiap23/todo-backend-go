...
The app will be available at http://localhost:8080. The `app` service reads configuration from `.env`. To run migrations on first startup (when `BACKEND_PRIMARY.ENV` is not `local`), set `BACKEND_PRIMARY.ENV=development` in `.env`, or run migrations manually inside the container.

## Configuration

Configuration is managed through environment variables with the `BACKEND_` prefix:
